...
def bmi():
  ...
  data = UserDetails.query.filter_by(
    user_id=current_user.id).first()  # type: ignore

  if request.method == "POST" and form.validate():
    height = form.height.data
    weight = form.weight.data
    if data is None:
      element = UserDetails(user_id=current_user.id,
                            height=height,
...
def water():
  ...
  waters = WaterIntake.query.filter_by(user_id=current_user.id).order_by(
    WaterIntake.date.desc()).all()  # type: ignore
  ...
  if request.method == "POST" and form.validate():
    ...
    element = WaterIntake(user_id=current_user.id,
                          water_ml=amount)  # type: ignore
    db.session.add(element)
    db.session.commit()
    flash("Your water intake has been updated.", "success")
    return redirect(url_for("water"))
  return render_template("water.html", waters=waters, form=form,
...
def water_delete_all():
  waters = WaterIntake.query.filter_by(
    user_id=current_user.id).all()  # type: ignore
  for water in waters:
    db.session.delete(water)
  db.session.commit()
  flash("Your water intake has been deleted.", "success")
...
def food():
  ...
  form.food.choices = [(food.food_no,
                        food.food_name + " " + str(food.food_cal))
                       for food in food_list]

  if request.method == "POST" and form.validate():
    food_no = form.food.data
    element = FoodIntake(user_id=current_user.id,
                         food_no=food_no)  # type: ignore
    db.session.add(element)
    db.session.commit()
  ...
  return render_template("food.html", form=form, foods=foods,
                         total=total)  # type: ignore
...
@login_required
def food_delete_all():
  foods = FoodIntake.query.filter_by(
    user_id=current_user.id).all()  # type: ignore
  for food in foods:
    db.session.delete(food)
  db.session.commit()
